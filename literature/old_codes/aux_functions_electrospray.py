...
class ElectrosprayClassification:
    ...
    def do_monica(self, max_value_of_the_data, quantity_max_data, percentage_max, flow_rate, fft_max_peaks_array,
                  cont_fft_max_peaks):

        # todo: use of fft_max_peaks_array (defined in the function calculate_peaks_fft of electrospray.py)

        # PEAKS SIGNAL
        if max_value_of_the_data > 1500:
            if (flow_rate / (2.7778e-7 * 10e-6)) < 100:  # uL/h
                if percentage_max > 0.05:
                    return "streamer onset"
                if quantity_max_data > 20000:
                    return "streamer onset"
