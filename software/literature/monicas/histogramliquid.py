...
def calculate_for_every_shape(shape):
    ...
    mean_histogram[index_aux - 1].append(mean_value_array_aux)
...
x1 = np.linspace(-20, 250, endpoint=True, num=50)  # num is number of points in array, default is 50
ax.set_xlim(-20, 250)
...
for i in range(0, index_aux):
    ...
    sns.histplot(data_each_shape[i][0], bins=x1, color=colors[i], kde=True)
...
plt.savefig("C:/Users/hvvhl/Desktop/joao/EHDA_library/plot_generated/histogram_mean_each_shape" + str(labels_name)+".jpg", format='jpg', dpi=300)
# ax.yaxis.set_major_formatter(ticker.PercentFormatter(xmax=len(name)))


fig, axs = plt.subplots(index_aux)
# plt.axvline(mean_value_array, color='k', linestyle='dashed', linewidth=1)
x1 = np.linspace(-20, 250, endpoint=True, num=50)  # num is number of points in array, default is 50
ax.set_xlim(-20, 250)
for i in range(0, index_aux):
    axs[i].set(xlabel="Current [nA]")
    axs[i].set_title(name_liquid + ' - ' + name[i])
    axs[i].plot(data_each_shape[i][0], color=colors[i])
    # ax.plot(mean_each_shape[i][0])
    # ax.axhline(mean_value_array[i], color=colors[i], lw=2, alpha=0.5)
# ax.hist(mean_div_max_variation_array, bins=len(current_shapes), edgecolor='black', alpha=0.3)
ax.set_title("Histogram and vertical line with the mean of 50k values for each shape of " + name_liquid, fontsize=10)
...
x1 = np.linspace(0, 175, endpoint=True, num=30)  # num is number of points in array, default is 50
ax.set_xlim(-5, 185)

for i in range(0, index_aux):
    ax.hist(mean_histogram[i][0], color=colors[i], bins=x1)
ax.set_xlabel("Value current [nA]")
ax.set_ylabel("Count of the mean for each measurement ")
plt.title('Mean histogram of 10 measurements for each shape of ' + name_liquid)
# ax.label(labels_name)
ax.legend(loc='best')
handles = [Rectangle((0, 0), 1, 1, color=c, ec="k") for c in colors]
ax.legend(handles, labels_name)
plt.rcParams["figure.figsize"] = (20, 12)
plt.savefig("C:/Users/hvvhl/Desktop/joao/EHDA_library/plot_generated/histogram_mean_10measurements" + str(labels_name)+".jpg", format='jpg', dpi=300)
