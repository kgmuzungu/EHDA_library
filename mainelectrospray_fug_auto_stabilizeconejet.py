...
number_measurements = 50
# """
Q = 540   # flow rate  uL/h **********************************************************************
...
# *********************************************************************************************
name_setup = "setup7"
setup = "C:/Users/hvvhl/PycharmProjects/EHDA/setup/" + name_setup
...
def step_sequency(obj_fug_com, step_size=300, step_time=1, step_slope=0, voltage_start=0, voltage_stop=100, stop=False):
    ...
    if scp:
        ...
        step_sequency_thread = threading.Thread(target=step_sequency, name='step sequency FUG',
                                                args=(
                                                    ...
                                                    voltage_stop, lambda : stop_threads, ))
        ...
        try:
            ...
            for j in range(number_measurements):
                ...
                classification_monica = electrospray_classification.do_monica(max_data, quantity_max_data,
                                                        percentage_max, Q, max_fft_peaks, cont_max_fft_peaks)
                ...
                if current_shape["Sjaak"] == "cone jet ": # and current_shape["Monica"] == "no streamer onset":
                    count_sequency_cone_jet = count_sequency_cone_jet + 1

                    if count_sequency_cone_jet > 3:
                        stop_threads = True

                        if voltage_from_PS != 0:
                            str_voltage = str(voltage_from_PS)
                            print(FUG_sendcommands(obj_fug_com, ['U ' + str_voltage]))
                            # threads finish
                            if MODERAMP:
